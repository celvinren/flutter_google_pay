...
class FlutterGooglePay {
  static const MethodChannel _channel = const MethodChannel("flutter_google_pay");
  ...
  static Future<Result> _call(String methodName, dynamic data) async {
    Result result = await _channel.invokeMethod(methodName, data).then((dynamic data) {
      ...
      return Result(error?.toString() ?? 'unknow error', null, ResultStatus.ERROR, (error?.toString()) ?? "");
    });
    if (result != null) {
      return result;
    }
...
bool _validateAmount(dynamic amount) {
  return (amount?.toString() ?? "").length > 0;
}

bool _validateCurrencyCode(dynamic currencyCode) {
  bool isNotEmpty = (currencyCode?.toString() ?? "").length > 0;
  if (!isNotEmpty) {
    return false;
  }
...
class PaymentBuilder {
  ...
  addGateway([String? gateway, String? gatewayMerchantId]) {
    if (_directTokenizationSpecification != null) {
      throw Exception("You already set a DIRRECT. You can use DIRECT or Gateway.");
    }
    Map gateway = Map();
    ...
    _gatewayTokenizationSpecification = {"type": "PAYMENT_GATEWAY", "parameters": gateway};
  }
  ...
  addDirectTokenizationSpecification(String directTokenizationPublikKey, {String protocolVersion = "ECv2"}) {
    if (_gatewayTokenizationSpecification != null) {
      throw Exception("You already set a gateway. You can use DIRECT or Gateway.");
    }
    if (isEmpty(directTokenizationPublikKey)) {
      throw Exception("Please add protocol version & public key.");
    }
    Map directTokenizationParameters = {"protocolVersion": protocolVersion, "publicKey": directTokenizationPublikKey};
    _directTokenizationSpecification = {"type": "DIRECT", "parameters": directTokenizationParameters};
  }
  ...
  addAllowedCardNetworks(List<String> allowedCardNetworks) {
    if (allowedCardNetworks.length > 0) {
  ...
  addAllowedCardAuthMethods(List<String> allowedCardAuthMethods) {
    if (allowedCardAuthMethods.length > 0) {
  ...
  addShippingSupportedCountries(List<String> shippingSupportedCountries) {
    if (shippingSupportedCountries.length > 0) {
  ...
  Map get _cardPaymentMethod {
    ...
    if (_gatewayTokenizationSpecification != null || _directTokenizationSpecification != null) {
